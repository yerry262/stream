...
- `src/App.jsx` — the whole UI: search box + type filter chips, a "Search on:" row of deep links to streaming services, catalog grid, click-to-play overlay. Search matches title/description/year/genres; filtering is pure client-side over the in-memory catalog.
  ...
  - **Play bar** (`parseUrlInput`): paste a direct http(s) link to a media file (`.mp4/.mp3/.m3u8/…`) or a `magnet:` link and it plays in the overlay without a catalog edit. `.torrent` URLs are redirected to the "Open .torrent" file picker (cross-origin fetches of .torrent files usually fail CORS).
  - **In-browser torrents** (`src/torrent.js` + `TorrentPlayer`): a full WebTorrent client runs in the page — no install. Magnet links and .torrent files stream their largest media file (video or audio) into the player; "Seed a file" seeds a local media file and yields a shareable magnet (seeding lasts while the tab is open). Streaming is progressive: a service worker (`public/sw.min.js`, copied from `webtorrent/dist/sw.min.js` — re-copy on webtorrent upgrades) turns the `<video>`'s range requests into piece priorities, so only parts around the playhead are fetched and seeking re-targets the download; if SW registration fails it falls back to download-then-play via `file.blob()`. WebTorrent is lazy-loaded (its own ~235 kB chunk) on first torrent use. **WebRTC-only caveat**: browser peers can only reach other WebRTC peers (another tab of this app, WebTorrent Desktop, webtorrent-hybrid) via the WSS trackers in `TRACKERS` — plain qBittorrent/Transmission seeds are not reachable. Torrent playback is for content users have the right to share (home videos, CC/public-domain).
